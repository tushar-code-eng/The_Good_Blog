...
app = Flask(__name__)
app.secret_key='very-secrete-key'
...
def post_route(post_slug):
    ...
    return render_template('poste.html', params=params, posts=posts)
...
def login():
    if ('user' in session and session['user'] == params['admin_username']): #this is to check if userr is logged in
...
def edit(sno):
    if ('user' in session and session['user'] == params['admin_username']): #this is to check if userr is logged in
        if request.method == 'POST':
            ...
            date = datetime.now()
            if sno == '0':
                post = Posts(title=box_title, slug=slug, content=content, subtitle=tline, img=img_file, date=date)
                db.session.add(post)
                db.session.commit()
            else:
